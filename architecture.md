...
>
> 状态：2026-07 确立。三层架构已落地，关联影响分析 MVP 已实现（见 §6.2）。
> 阶段二 asset_profile / reverse_index 待跨资产需求驱动时再建（避免死代码）。
...
| 2 | build_asset_profile()：从 knowledge + raw + catalog 提炼资产全貌 | ⏸ 待跨资产需求驱动 |
| 3 | build_reverse_index()：反向血缘索引 | ⏸ 待跨资产需求驱动 |

> asset_profile / reverse_index 暂不建骨架：当前影响分析直接读 knowledge，
> 没有实际消费者。等跨资产影响分析需求启动时再建，避免死代码。
...
| asset_profile 时机 | 待跨资产需求驱动，不提前建骨架 | 当前影响分析直接读knowledge，无消费者；避免死代码 |
