...
>
> 状态：2026-07 确立，正在落地中。关联影响分析 MVP 设计已于 2026-07-09 定稿（见 §6.2），待实施。
...
| 步骤 | 内容 | 产出 |
|------|------|------|
| 1 | 输入解析：读 changes yaml → ChangeItem[] | 变更列表 |
| 2 | 三层过滤：表级命中 → 字段级命中 → 丢弃 | 命中变更 |
| 3 | 逐跳传播：沿 field_mappings 抽列依赖、跨跳别名对齐 | 波及目标字段集 |
| 4 | 影响判定：每跳表达式 + 目标DDL → severity + reason | 影响清单 |
| 5 | Excel 渲染：三 Sheet 输出 | impact.xlsx |
| 6 | command：impact-analysis.md | 可调用入口 |
| 7（未来）| AI 解读层：清单 → 报告 | 影响报告 |
...
### 阶段三：关联影响分析（设计已定稿，见 §6.2）

| 步骤 | 内容 |
|------|------|
| 1 | 输入解析：changes yaml → ChangeItem[] |
| 2 | 三层过滤：表级命中 → 字段级命中 → 丢弃 |
| 3 | 逐跳传播：沿 field_mappings 抽列依赖、跨跳别名对齐 |
| 4 | 影响判定：表达式 + 目标DDL → severity + reason |
| 5 | Excel 渲染：三 Sheet（影响清单/表级影响/过滤摘要） |
| 6 | impact-analysis.md command |
| 7（未来）| AI 解读层：清单 → 报告 |
