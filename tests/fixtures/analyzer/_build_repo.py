...
def build_mock_repo(base_dir):
    ...
    # 真实格式特征（用户确认的标准格式）：
    # 1. 字段名带*前缀用单引号包裹（'*任务名称'），yaml.safe_load 能解析
    # 2. 冒号后有空格（规范 YAML）
    # 3. 额外信息: | 的 block scalar 吞掉 Jobs 和 taskParams
    # 4. Jobs 和 taskParams 在 block scalar 内，需要二次提取
    ...
    other_lts_dir.mkdir(parents=True, exist_ok=True)
    (other_lts_dir / "TASK_OTHER_F.yml").write_text(
        yaml.dump({
            "*任务名称": "TASK_OTHER_F",
            "调度周期": "0 0 3 * * ?",
            "taskParams": [
                {"*参数名称": "V_GROUP_CODE", "参数值": "GR_OTHER_NOT_MATCHED"},
            ],
        }, allow_unicode=True, sort_keys=False),
        encoding="utf-8")
